...
void main() {
  group('FileBlobStore', () {
    ...
    group('archiveKey', () {
      test('generates correct key format', () {
        ...
        expect(key, equals('packages/my_package/1.0.0/abc123.tar.gz'));
      });

      test('handles package names with underscores', () {
        final key = blobStore.archiveKey('my_great_package', '2.0.0', 'def456');

        expect(key, equals('packages/my_great_package/2.0.0/def456.tar.gz'));
      });
      ...
      test('handles prerelease versions', () {
        ...
        expect(key, equals('packages/test_pkg/1.0.0-beta.1/sha256hash.tar.gz'));
      });
      ...
      test('handles build metadata versions', () {
        ...
        expect(
            key, equals('packages/test_pkg/1.0.0+build.123/sha256hash.tar.gz'));
      });
    });

    group('putArchive', () {
      test('writes file to correct location', () async {
        final key = 'packages/test_pkg/1.0.0/hash.tar.gz';
      ...
      test('creates nested directories', () async {
        final key = 'packages/deeply/nested/package/1.0.0/hash.tar.gz';
      ...
      test('handles empty data', () async {
        final key = 'packages/empty_pkg/1.0.0/hash.tar.gz';
      ...
      test('handles large files', () async {
        final key = 'packages/large_pkg/1.0.0/hash.tar.gz';
        // Create 1MB of data
    ...
    group('getArchive', () {
      ...
      test('throws on missing file', () async {
        final key = 'packages/nonexistent/1.0.0/hash.tar.gz';
    ...
    group('exists', () {
      ...
      test('returns false for non-existing directory', () async {
        final key = 'packages/no/such/path/hash.tar.gz';
    ...
    group('getDownloadUrl', () {
      ...
      test('handles base URL with trailing slash', () async {
        ...
        final key = 'packages/my_package/2.0.0/hash.tar.gz';
  ...
  group('FileBlobStore as cache', () {
    ...
    test('cache store works the same as regular store', () async {
      final key = 'packages/cached_pkg/1.0.0/hash.tar.gz';
      final data = Uint8List.fromList([1, 2, 3, 4, 5]);
  ...
  group('Archive key generation edge cases', () {
    ...
    test('handles very long package names', () {
      ...
      expect(key, startsWith('packages/$longName'));
    });
  ...
  group('File operations concurrency', () {
    ...
    test('handles concurrent writes to different keys', () async {
      ...
      for (var i = 0; i < 10; i++) {
        final key = 'packages/pkg_$i/1.0.0/hash.tar.gz';
