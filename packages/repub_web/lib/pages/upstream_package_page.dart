...
class UpstreamPackagePage extends StatefulComponent {
  ...
  const UpstreamPackagePage({required this.packageName, super.key});

  @override
  State<UpstreamPackagePage> createState() => _UpstreamPackagePageState();
}

class _UpstreamPackagePageState extends State<UpstreamPackagePage> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadPackage() async {
    ...
    try {
      final info = await apiClient.getUpstreamPackage(component.packageName);
      setState(() {
        _packageInfo = info;
        _loading = false;
      });
  ...
  Component _buildErrorState(String error) {
    return div(
      ...
      [
        ...
        a(
          href: '/upstream-packages/${component.packageName}',
