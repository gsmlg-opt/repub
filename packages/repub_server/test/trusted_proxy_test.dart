...
void main() {
  group('Trusted Proxies', () {
    ...
    setUp(() async {
      ...
      blobs = FileBlobStore(basePath: '/tmp/blobs', baseUrl: 'http://localhost');
      cacheBlobs = FileBlobStore(basePath: '/tmp/cache', baseUrl: 'http://localhost', isCache: true);
    ...
    test('trusts X-Forwarded-For when socket IP is in trusted proxies', () async {
      final config = Config(
        listenAddr: '0.0.0.0',
        listenPort: 4920,
      ...
      expect(loginResponse.statusCode, equals(400)); // Will fail auth but won't crash on IP extraction
    });
  });
}
